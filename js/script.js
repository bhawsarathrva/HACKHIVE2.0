...
(function () {
  ...
  // --- Continuous High-Precision Fluid RAF Loop ---
  function render(time) {
    if (isInitialized) {
      // Snappy high-response fluid follow (0.58) - zero drag feel with silky organic deceleration
      ...
      // Hardware GPU compositor update
...
(function () {
  ...
  // Honeycomb geometry configuration
  const HEX_RADIUS = 30;         // hexagon radius
  const GAP = 4.5;               // spacing between hexagons
  const R = HEX_RADIUS + GAP;
  const GLOW_RADIUS = 210;       // cursor influence radius in px
  ...
  // Precompute flat-topped hexagon vertex offsets
  const corners = [];
  for (let i = 0; i < 6; i++) {
  ...
  const BASE_FILL = 'rgba(239, 169, 58, 0.035)';
  ...
  function renderHoneycomb() {
    ...
    // Warm, crisp white-to-cream gradient base
    const bgGrad = ctx.createLinearGradient(0, 0, 0, H);
    bgGrad.addColorStop(0, '#ffffff');
    bgGrad.addColorStop(0.65, '#fffdf8');
    bgGrad.addColorStop(1, '#fcf7ec');
    ctx.fillStyle = bgGrad;
    ctx.fillRect(0, 0, W, H);

    // Draw ambient golden spotlight under cursor
    ...
    // Render hexagons with instant cursor tracking and smooth glow bloom/decay
    for (let i = 0; i < hexCells.length; i++) {
      ...
      if (distSq < GLOW_RADIUS_SQ) {
        ...
        targetGlow = factor * factor; // Quadratic falloff for bright focal center
      }

      // Fast bloom on cursor entry, smooth decay on exit
      ...
      if (cell.glow > 0.01) {
        ...
        // Very subtle idle breathing pulse
      ...
      // Radiant inner core highlight for close hexagons
      if (cell.glow > 0.35) {
        drawHexagon(cell.x, cell.y);
